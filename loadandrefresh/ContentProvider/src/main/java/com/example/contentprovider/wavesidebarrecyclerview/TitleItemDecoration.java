...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private List<SortModelBean> mData;
    ...
    public TitleItemDecoration(Context context, List<SortModelBean> data) {
        super();
        mData = data;
        mPaint = new Paint();
        mBounds = new Rect();
        mTitleHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 30, context.getResources().getDisplayMetrics());
        Log.e("zzf" ,"mTitleHeight" + mTitleHeight);
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0的时候绘制title
                    ...
                    if (null != mData.get(position).getSortLetters() && !mData.get(position)
                            .getSortLetters().equals(mData.get(position - 1).getSortLetters())) {
    ...
    private void drawTitle(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        int top1 = params.topMargin;
        c.drawRect(left, top, right, bottom, mPaint);
        Log.e("zzf","left"+ left +"---"+ "top" + top +"-----"+"right" + right +"-----"+ "bottom" + bottom +"----"+ "top1" + top1);

        mPaint.setColor(TITLE_TEXT_COLOR);
        //将文本放入到一个矩形框中，并测量文本的宽和高
        mPaint.getTextBounds(mData.get(position).getSortLetters(), 0, mData.get(position).getSortLetters().length(), mBounds);
