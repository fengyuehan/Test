...
public class ImageLoader implements ImageBase {
    ...
    @Override
    public void displayImage(String imageUri, ImageView imageView) throws IOException {
        switch (Scheme.ofUri(imageUri)) {
            case FILE:
                displayImageFromFile(imageUri, imageView);
                return;
            case ASSETS:
                displayImageFromAssets(imageUri, imageView);
                return;
            case DRAWABLE:
                displayImageFromDrawable(imageUri, imageView);
                return;
            case HTTP:
            case HTTPS:
                displayImageFromNetwork(imageUri, imageView);
                return;
            case CONTENT:
                displayImageFromContent(imageUri, imageView);
                return;
            case UNKNOWN:
            default:
                Matcher m = NUMBER_PATTERN.matcher(imageUri);
                if (m.matches()) {
                    displayImageFromResource(Integer.parseInt(imageUri), imageView);
                    return;
                }
                displayImageFromOtherSource(imageUri, imageView);
                return;
        }
    }
